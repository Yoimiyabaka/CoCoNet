...
import math
from myProCon import MutObjNew


class MutualInformation20:
    __group_flag = False
    __g_percent = 0.0
    __group = []
    __p_1 = 0.0
                   ...
                   self.__freqaa[j][20] < self.__paraml:
                    ...
                    for k in range(self.__sequence_number):
                        if self.__seq[k][j] == '-' or self.__seq[k][i] == '-':
                            ...
                            ii = self.__l_table[ord(self.__seq[k][i]) - ord('A')]
                            jj = self.__l_table[ord(self.__seq[k][j]) - ord('A')]
                            if ii >= 0 and jj >= 0:
                    ...
                    for ii in range(20):
                        for jj in range(20):
                            if self.__prob_prob[ii][jj] != 0.0:
                                ...
                                probmutu_tmp[ii][jj] = self.prob_mutu_calc(a, b, self.__freqaa[i][ii], self.__freqaa[j][jj])
    ...
    def display_mutu_info(self):
        for i in range(self.__residus_number):
            for j in range(self.__residus_number):
                print("i = {} j = {} {}".format(i, j, self.__mutuinf[i][j]))
    ...
    def mut_inf_to_file_20(self, file):
        fp = open(file, 'w')
        mut_inf = self.__mutuinf
        for i in range(len(mut_inf)):
            for j in range(len(mut_inf[i])):
                fp.write("{},".format(mut_inf[i][j]))
            fp.write("\n")
        fp.close()
